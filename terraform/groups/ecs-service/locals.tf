...
locals {
  ...
  service_name                = "acsp-profile-delta-consumer"
  container_port              = "8080"
  docker_repo                 = "acsp-profile-delta-consumer"
  kms_alias                   = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path            = "/acsp-profile-delta-consumer/healthcheck"
  healthcheck_matcher         = "200"
  vpc_name                    = local.stack_secrets["vpc_name"]
  s3_config_bucket            = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename    = "acsp-profile-delta-consumer.env"
